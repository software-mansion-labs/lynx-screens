...
- [x] `696309396` M hdr-ios — chore: Separate header item building from HeaderItem(Spacer)ComponentView ([#4135](https://github.com/software-mansion/react-native-screens/commit/696309396)) — `RNSStackHeaderContentFactory` + `RNSStackHeaderItem(Spacer)DataProviding` protocols; `customView` returns the item's painting view (`self.view`), not the component itself; pod deployment target raised to 15.1 (RNS minimum) so UIKit bar-button/menu/large-title APIs are used verbatim; verified on simulator (no regression)
