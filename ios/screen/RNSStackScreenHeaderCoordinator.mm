...
- (void)rebuild
{
    ...
    // Adaptation: on Lynx the config's children are LynxUI components, not
    // views - iterate them as `id`; the protocol conformance checks stay the
    // same as in RNS.
    for (id child in _configDataProvider.children) {
            ...
            parentMenu:(nullable RNSStackHeaderMenuData *)parentMenu
{
    ...
    if (singleSelectionRoot != nil) {
        ...
        toggleIds =
            parentMenu != nil ? [RNSStackHeaderMenuCoordinator getToggleItemsIdsInMenu:parentMenu] : @[ elementId ];
...
{
    NSAssert(_screenController != nil, @"[RNScreens] Screen Controller cannot be nil");
...
#if !TARGET_OS_TV
    navItem.largeTitleDisplayMode = _configDataProvider.largeTitleEnabled
